...
```{r}
view(stroopdatWRespLat <- stroopdatWRespLat %>% group_by(id, congruency) %>%
       summarise(., rtLatMean = mean(rtLatency)))
```
...
view(stroopdatWRespLat <- stroopdatWRespLat %>%
       group_by(id) %>%
       mutate(., SIErtLatency = rtLatMean - lag(rtLatMean)))
```
...
view(stroopSummRTLatency <- stroopdatWRespLat %>%
       group_by(congruency) %>%
       summarise(meanrtLat = mean(rtLatMean)))
```
...
```{r}
ggplot(data = giantstroop,
       mapping = aes(y=rtLatMean, x=congruency, fill = congruency)) +
  geom_boxplot()+
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) + geom_point()
```
